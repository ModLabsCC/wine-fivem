...
static BYTE fivem_adhesive_writebuf[64];
static __thread ULONG_PTR fivem_adhesive_assert_loop_rip;
static __thread ULONG_PTR fivem_adhesive_assert_loop_rsp;
static __thread unsigned int fivem_adhesive_assert_loop_count;
...
static inline ULONG_PTR get_fivem_adhesive_synth_state(void)
{
    ...
    memcpy( fivem_adhesive_synth_state + 0x44, &self_ptr, sizeof(self_ptr) );
    fivem_adhesive_synth_state[0x08] = 0x07;
    fivem_adhesive_synth_state[0x0c] = 0x07;
    fivem_adhesive_synth_state[0x27] = 0x01;
    fivem_adhesive_synth_state[0x42] = 0x07;
    fivem_adhesive_synth_state[0x23] = 0x07; // 0x27 %rdx -> synth_state[0x23]
    fivem_adhesive_synth_state[0x3e] = 0x07; // 0x42 %rdx -> synth_state[0x3e]
    fivem_adhesive_synth_state[0x59] = 0x07; // 0x5d %rdx -> synth_state[0x59]
    memcpy( fivem_adhesive_synth_state + 0x44, &self_ptr, sizeof(self_ptr) );
...
static BOOL handle_fivem_adhesive_ud2_hack( ucontext_t *sigcontext, EXCEPTION_RECORD *rec )
{
    ...
    {
        BYTE sig[8];
        if (virtual_uninterrupted_read_memory( (BYTE *)rip, sig, 8 ) == 8 &&
            sig[0] == 0x0f && sig[1] == 0x0b)
        {
            /* If we see the signature of the array out of bounds assert function ... */
            if (sig[2] == 0x0f && sig[3] == 0x0b &&
                sig[4] == 0x45 && sig[5] == 0x31)
            {
                if (fivem_adhesive_signal_debug_enabled())
                    fprintf( stderr, "wine[fivem-adhesive]: gracefully truncating adhesive array overflow by advancing to epilogue from %p\n", (void *)rip );

                /* The true epilogue consists of:
                 * addq $0xd8, %rsp
                 * popq %rdi
                 * popq %rsi
                 * jmp 0x6ffff1e62700 (fatal reporter)
                 * We must emulate the stack teardown and return natively to bypass the fatal reporter entirely. */
                ULONG_PTR *stack = (ULONG_PTR *)RSP_sig(sigcontext);
                stack = (ULONG_PTR *)((BYTE *)stack + 0xd8); // addq $0xd8, %rsp
                RDI_sig(sigcontext) = *stack++;              // popq %rdi
                RSI_sig(sigcontext) = *stack++;              // popq %rsi
                RIP_sig(sigcontext) = *stack++;              // retq (pop return address into RIP)
                RSP_sig(sigcontext) = (ULONG_PTR)stack;
            ...
            /* Generic fallback: skip any other UD2 instruction (e.g. from adhesive JIT shellcode on the stack).
             * This is critical for stack-based integrity check shellcode that traps on failure. */
            if (fivem_adhesive_signal_debug_enabled())
                fprintf( stderr, "wine[fivem-adhesive]: skipping generic UD2 at %p (sig: %02x %02x %02x %02x %02x %02x)\n",
                         (void *)rip, sig[2], sig[3], sig[4], sig[5], sig[6], sig[7] );
            RIP_sig(sigcontext) = rip + 2;  /* skip past UD2 (0f 0b = 2 bytes) */
